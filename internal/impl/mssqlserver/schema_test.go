...
func TestMssqlTypeNameToCommonType(t *testing.T) {
	tests := []struct {
		...
		{"REAL", schema.Float32},
		// Decimal types: preserve precision as string
...
// TestSchemaCacheInvalidation verifies that direct deletion from the cache
// removes the entry (used to model DDL-triggered invalidation if added later).
func TestSchemaCacheInvalidation(t *testing.T) {
	b := &batchPublisher{tableSchemas: make(map[string]any)}

	b.tableSchemas["orders"] = map[string]any{"name": "orders"}
	b.tableSchemas["products"] = map[string]any{"name": "products"}

	// Invalidate one table
	b.tableSchemasMu.Lock()
	delete(b.tableSchemas, "orders")
	b.tableSchemasMu.Unlock()

	// orders should be gone; products should remain
	assert.Nil(t, b.getOrComputeTableSchema("orders", nil, nil))
	require.NotNil(t, b.getOrComputeTableSchema("products", nil, nil))
}
